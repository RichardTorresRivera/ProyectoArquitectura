/*
  OLED_I2C.cpp - Multi-platform library support for 128x32 and 128x64 pixel SSD1306 OLEDs
  ...
  based on the SSD1306 controller chip with an Arduino, chipKit, ESP8266 or ESP32.
...
void OLED::drawFillCircle(int x, int y, int radius)
{
	int f = 1 - radius;
	int ddF_x = 1;
	int ddF_y = -2 * radius;
...
void OLED::clrFillCircle(int x, int y, int radius)
{
	int f = 1 - radius;
	int ddF_x = 1;
	int ddF_y = -2 * radius;
	...
	while(x1 < y1)
	{
		...
		ddF_x += 2;
		f += ddF_x;    
		clrLine(x + x1, y + y1, x - x1, y + y1);
		clrLine(x + x1, y - y1, x - x1, y - y1);
		clrLine(x + y1, y + x1, x - y1, y + x1);
		clrLine(x + y1, y - x1, x - y1, y - x1);
	}
}
